...
全局设置和「本会议的 AI 设置」都只配置一个模型连接，校对和总结共用提供方、区域或代理地址与 Key，分别选择 Model ID 和推理强度。全局默认值用于新会议；旧会议可点击「采用当前全局默认值」。更改配置不改写已有校对或纪要版本。
...
共用一处「验证模型调用」，检查校对与总结配置；模型 ID 和推理强度相同时只调用一次。可验证未保存的 Key；留空则读取该地址已保存的 Key。检查发送固定测试文字，会产生少量推理用量，不发送会议内容。「保存并关闭／保存」用于保存模型配置；输入框里的 Key 需要单独点击「保存 API Key」才能供后续处理使用。
...
离线测试覆盖旧配置解码、自定义 ID、可选 reasoning、URL 规范化、Key 端点隔离、SigV4 区域、Bearer 请求、代理响应与用量、HTTP 失败与未完成响应、禁止应用层重试／回退、不同阶段共用连接、独立模型参数及版本保存。第三方实际兼容性需在设置中用用户自己的服务验证。

2026-09-19：全套 186 项测试通过（Core 69、Cloud 85、Audio 32）；本地 App 编译与签名验证通过。界面验证确认全局／单会议入口、提供方切换、完整请求地址预览、自定义 Model ID、服务默认推理强度及未配置 Key 时禁用调用检查。界面测试使用临时配置后取消，未改变现有会议设置，也未发送真实推理请求。
...
模型连接统一配置一次，保存和后续工作流都会将同一连接用于两个阶段。现有 Key 沿用按端点保存的钥匙串条目，无需迁移密钥；历史校对和纪要快照保持原样。

纪要按钮上方显示将复用的校对版本号，选择规则与实际生成共用同一实现：取与当前转录修订匹配的最新完整校对，跳过未完成版本；没有匹配版本时先校对再总结。待确认的修改继续使用修改前文本。每个历史纪要另显示生成时关联的校对版本，明确区别于下一次生成的来源；跳过校对的版本也单独标明。

本次共享连接与校对来源更新：全套 189 项测试通过（Core 71、Cloud 86、Audio 32）。测试覆盖共用连接时保留各阶段模型参数、最新完整校对的选择、历史版本号稳定，以及实际生成关联的校对 ID 与界面预览一致。

最后补充了保持等价旧设置字段的处理，4 项相关回归再次通过。App 已更新并通过签名检查；实际界面确认代理提供方、URL、Key 和验证按钮均只出现一处，保存的 Key 状态保持可用，纪要页同时显示下一次生成和当前纪要所关联的校对版本号。
