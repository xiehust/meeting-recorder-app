...
1. 添加中文、日文或英文条目，填写词条和可选输出写法。备注仅存在本机。启用开关控制下次同步的内容。
...
单一语言通过 `VocabularyName` 传入相应词汇表；中英混合／英日混合通过 `VocabularyNames` 传入匹配语言的词汇表；中英混合不会加载日文词汇表，英日混合不会加载中文词汇表。日文固定识别使用 `ja-JP`，支持汉字、平假名和片假名词条，语言规则由 AWS 进一步校验。麦克风与会议应用两路都使用相同配置。
