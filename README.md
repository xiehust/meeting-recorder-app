...
根据 [PRD](PRD.md) 开发的原生 SwiftUI 菜单栏应用。当前为 **0.3.5 开发预览：原生记录、转录、AI 校对与可自定义模板的纪要**，不是 PRD 全部功能的完成版。
...
用户已在 0.1.1 验证 Teams 耳机会议的两路采集及转录。Zoom、飞书、腾讯会议、钉钉的真实通话，以及混合语言与说话人分离组合、设备热切换、外放回声和两小时稳定性仍需按 [M0 验证记录](docs/M0-VALIDATION.md) 实测。断网后支持继续缓存和标记区间，但自动重连、缓存补转、音频回听及到期清理尚未实现；恢复实时转录请暂停再恢复。当前不使用 S3。
