...
[PRD](PRD.md) に基づく、SwiftUI 製のネイティブなメニューバーアプリです。**0.6.0 は開発プレビュー版**で、中国語・英語・日本語の UI、リアルタイム文字起こし、会議後の録音確認、AI 校正、カスタマイズ可能な要約に対応しています。PRD の全機能が完成したものではありません。
...
表示言語は認識言語・要約言語とは独立しています。**リアルタイム・バッチ音声認識は中国語・日本語・英語、従来の中国語と英語の混在、新しい中国語・日本語・英語の混在に対応します。要約出力とカスタム語彙も 3 言語に対応します。** 表示言語を変更しても、これらの設定や過去の要約は変わりません。macOS が管理するメニュー、ファイル選択画面、権限ダイアログ、Finder の名前はシステムの言語設定に従います。[多言語対応の詳細](docs/LOCALIZATION.md)（中国語）も参照してください。
...
- 2 系統の AWS Transcribe Streaming を使用。音声形式は 16 kHz、16-bit、モノラル PCM、100 ms 単位です。混在モードは `IdentifyMultipleLanguages` と `zh-CN,en-US` または `zh-CN,en-US,ja-JP`、固定言語モードは各言語コードを使用し、相手側の音声には話者ラベルを要求します。
